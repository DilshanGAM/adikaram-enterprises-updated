...
export default function StaffLayout({
        ...
        if(!token){
            ...
            axios.get("/api/auth/user", {
                ...
                if(res.data.message === "User Found"){
                    ...
                    if(res.data.user.role === "staff" || res.data.user.role === "manager" || res.data.user.role === "admin"){
                        setStatus("success");
                    }else{
                        setStatus("error");
                    }
    ...
    return(
        <div className="w-full h-screen max-h-screen">
            ...
            {status=="error"&&<div className="flex items-center justify-center h-full">You are not authorized to view this page</div>}
        </div>
    )
}
